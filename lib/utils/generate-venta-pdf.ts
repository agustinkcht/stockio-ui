 ...
 */
export async function renderVentaPage(doc: jsPDF, venta: Venta, miNegocio: MiNegocio): Promise<void> {
 ...
 */
export async function downloadVentasPDF(ventas: Venta[], miNegocio: MiNegocio): Promise<void> {
  const doc = new jsPDF({ unit: "mm", format: "a4", orientation: "portrait" })
  for (let i = 0; i < ventas.length; i++) {
    if (i > 0) doc.addPage()
    await renderVentaPage(doc, ventas[i], miNegocio)
  }
  const filename = ventas.length === 1
    ? `Venta-${ventas[0].id}.pdf`
    : `Ventas-${new Date().toISOString().slice(0, 10)}.pdf`
  doc.save(filename)
}
