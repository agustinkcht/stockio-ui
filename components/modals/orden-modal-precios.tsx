...
interface OrdenModalPreciosProps {
  isOpen: boolean
  onClose: () => void
...
const DIRECTION_OPTIONS: Record<SortFactor, { label: string; value: SortDirection; description: string }[]> = {
  ...
  costo: [
    { label: "Mayor monto", value: "desc", description: "Mayor costo primero" },
    { label: "Menor monto", value: "asc", description: "Menor costo primero" },
  ],
  margen: [
    { label: "Mayor porcentaje", value: "desc", description: "Mayor margen primero" },
    { label: "Menor porcentaje", value: "asc", description: "Menor margen primero" },
  ],
  precioFinal: [
    { label: "Mayor monto", value: "desc", description: "Mayor precio primero" },
    { label: "Menor monto", value: "asc", description: "Menor precio primero" },
  ],
}

// Only include pricing-relevant factors
const AVAILABLE_FACTORS: SortFactor[] = ["titulo", "categoria", "marca", "costo", "margen", "precioFinal"]

const DEFAULT_PRIORITIES: SortFactorConfig[] = [{ factor: "categoria", direction: "asc" }]

export function OrdenModalPrecios({ isOpen, onClose, onApply, initialPriorities }: OrdenModalPreciosProps) {
  ...
  const inactiveFactors: SortFactor[] = AVAILABLE_FACTORS.filter((factor) => !activeFactor.has(factor))
  ...
  const addFactor = (factor: SortFactor) => {
    const defaultDirection = factor === "costo" || factor === "margen" || factor === "precioFinal" ? "desc" : "asc"
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Establece el orden de prioridad de los factores para ordenar los precios
            </p>
          </div>
          <button
            onClick={onClose}
