        ...
        <div
          className={`flex-1 relative overflow-hidden mb-0 ${gridSize === "lg" ? "h-22" : gridSize === "md" ? "h-16" : "h-10"} ${roundedClass} grid ${
            item.isAgrupador || item.hasVariants
              ? `grid-cols-11 ${isHovered ? "bg-white/15" : "bg-white/10"} backdrop-blur-xl border border-white/20 shadow-[0_8px_30px_rgba(0,0,0,0.12)] ring-1 ring-white/10 transition-colors cursor-pointer`
              : `grid-cols-11 ${isHovered ? "bg-white/15" : "bg-white/10"} backdrop-blur-xl border border-white/20 shadow-[0_8px_30px_rgba(0,0,0,0.12)] ring-1 ring-white/10 transition-colors`
          }`}
          onClick={(e) => {
