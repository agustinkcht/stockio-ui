...
type SortField = "fecha" | "total" | "cliente" | "estado"
type SortDir = "asc" | "desc"

const estadoConfig: Record<string, { bg: string; text: string; border: string; icon: typeof Clock; label: string }> = {
  en_curso:   { bg: "bg-amber-50",   text: "text-amber-700",   border: "border-amber-200",   icon: Clock,         label: "En Curso"   },
  finalizada: { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200", icon: CheckCircle2,  label: "Finalizada" },
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-slate-200 bg-white hover:border-slate-300 transition-colors text-sm font-medium text-slate-700 cursor-pointer shadow-sm"
      >
...
export default function VentasPage() {
  ...
  const { label: periodLabel } = usePeriodRange()
  ...
  const filtersActive = !!(filterEstado || filterCliente)

  // Unique clientes for filter dropdown
  const uniqueClientes = Array.from(new Set(ventas.map(getClienteNombre))).sort()
  ...
  const filteredVentas = ventas
    ...
    .sort((a, b) => {
      let cmp = 0
      if (sortField === "fecha") cmp = (a.fecha + a.hora).localeCompare(b.fecha + b.hora)
        ...
        <div className="flex-1 flex flex-col bg-white rounded-lg shadow-sm h-[calc(100vh-12px)] overflow-hidden relative z-10">
          ...
          <main className="flex-1 flex flex-col bg-[rgba(250,251,253,1)] overflow-hidden">
            ...
            <div className="sticky top-0 z-30">
              <div className="bg-[rgba(250,251,253,0.85)] backdrop-blur-md border-b border-slate-200/60">
                <div className="px-8 py-6 flex items-center justify-between gap-6">
                  ...
                  <button
                    ...
                    className="h-9 px-4 text-sm font-semibold transition-colors rounded-md flex items-center gap-2 bg-slate-900 text-white hover:bg-slate-800 cursor-pointer shrink-0 shadow-sm"
                  >
            ...
            <div className="flex-1 overflow-y-auto">
              <div className="px-8 pb-8 pt-6">
                ...
                {/* Search bar row — tab selector inline left */}
                <div className="flex items-center mb-5 gap-2">
                  {/* Tab selector as segmented control */}
                  <div className="flex items-center gap-1 bg-slate-100 rounded-md p-0.5 shrink-0">
                    {tabs.map(tab => {
                      const count =
                        tab.id === "todas" ? ventas.length :
                        tab.id === "en_curso" ? ventas.filter(v => v.estado === "en_curso").length :
                        tab.id === "finalizada" ? ventas.filter(v => v.estado === "finalizada").length :
                      ...
                      return (
                        ...
                        >
                          {tab.label}
                          {count > 0 && (
                            <span className={`text-[10px] px-1 py-0.5 rounded-full leading-none tabular-nums ${
                    ...
                    <button
                      ...
                      className={`h-8 text-xs transition-colors border shadow-sm gap-1.5 shrink-0 px-3 rounded-md flex items-center cursor-pointer ${
                        filtersActive
                          ...
                          : "border-[rgba(228,230,235,0.6)] hover:bg-gray-100 text-slate-600"
                      }`}
                    >
                  ...
                  {/* Sort — unified control like agregar productos */}
                  <div className="relative">
                    <div className={`flex items-center border shadow-sm rounded-md overflow-hidden bg-white ${showSort ? "border-slate-400" : "border-[rgba(228,230,235,0.6)]"}`}>
                      <button
                        type="button"
                        onClick={() => setSortDir(d => d === "asc" ? "desc" : "asc")}
                        className="h-8 px-2.5 hover:bg-slate-50 transition-colors border-r border-[rgba(228,230,235,0.6)] cursor-pointer"
                      ...
                      <select
                        ...
                        className="appearance-none h-8 pl-2.5 pr-7 text-xs bg-transparent focus:outline-none cursor-pointer text-slate-600"
                      >
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="grid grid-cols-100 min-h-[44px] pt-2 border-b border-slate-200/70">
                          ...
                          {/* Estado — was origen */}
                          <div className="col-span-10 flex items-center justify-start px-3 border-r border-slate-200/70">
                            <div className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-xs font-medium ${estadoStyle.bg} ${estadoStyle.text} ${estadoStyle.border}`}>
                              <EstadoIcon className="w-3 h-3 shrink-0" />
                              {estadoStyle.label}
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-100 min-h-[40px]">
                          ...
                          <div className="col-span-24 flex items-center px-3">
                            <button
                              ...
                              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 text-xs font-medium text-slate-700 truncate max-w-full ${
                                venta.cliente.tipo === "cuenta" ? "hover:bg-slate-200 cursor-pointer transition-colors" : "cursor-default"
                              }`}
                            >
