                ...
                {isEditable ? (
                  <div className="grid grid-cols-[1.9fr_0.8fr_auto_1fr_auto_1.1fr_1.1fr_1.5fr] h-9 text-xs font-medium text-slate-500 uppercase tracking-wider">
                    <div className="flex items-center px-4 gap-2">
                      ...
                      <button
                        onClick={() => {
                          // Pre-select existing items when opening selection modal
                  ...
                  <div className="py-16 text-center">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">Agrega items del proveedor para comenzar</p>
                      </div>
                      {isEditable && (
                        <button
                          onClick={() => {
                    ...
                    orden.items.map((item, idx) => {
                      ...
                      return (
                        <div key={idx} className="border-b border-slate-100">
                          {/* PLACEHOLDER - need to add the actual grid item row here */}
      ...
      {showAddItemModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4 overflow-hidden max-h-[85vh] flex flex-col">
            ...
            {selectionSearch.trim() && filteredProveedorItems.length === 0 && (
              <div className="px-4 py-3 bg-blue-50/50 border-b border-blue-100 flex-shrink-0">
                <button
                  onClick={handleAddFreeItem}
                  className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-blue-700 bg-blue-100 hover:bg-blue-200 rounded-lg transition-colors"
                >
                  <Plus className="w-4 h-4" />
                  Agregar &quot;{selectionSearch}&quot; como item libre
                </button>
              </div>
            )}
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredProveedorItems.length === 0 && !selectionSearch.trim() ? (
                <div className="py-12 text-center">
                  <p className="text-sm text-slate-500">No hay items asociados a este proveedor</p>
                </div>
              ) : filteredProveedorItems.length === 0 && selectionSearch.trim() ? (
                <div className="py-8 text-center">
                  <p className="text-sm text-slate-500 mb-2">No hay items que coincidan con &quot;{selectionSearch}&quot;</p>
                  <p className="text-xs text-slate-400">Puedes agregar este texto como item libre usando el boton de arriba</p>
                </div>
              ) : (
